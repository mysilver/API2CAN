...
class SwaggerParser(object):
    ...
    def __init__(self, swagger_path=None, swagger_dict=None, swagger_yaml=None, use_example=True, validate=False):
        ...
        for path, path_spec in self.specification['paths'].items():
            ...
            for http_method in path_spec.keys():
                ...
                if 'operationId' in action.keys():
                    ...
                    self.paths[path][http_method]['operationId'] = action['operationId']
                else:
                ...
                self.paths[path][http_method]['description'] = action.get('description')
                self.paths[path][http_method]['summary'] = action.get('summary')
                self.paths[path][http_method]['tags'] = action.get('tags')
        ...
        if 'type' in spec.keys():
            ...
            if 'type' in spec['schema'].keys() and spec['schema']['type'] == 'array':
                ...
                if '$ref' in spec['schema']['items']:
                    ...
                    return [self.build_example(spec['schema']['items'])]

            elif 'type' in spec['schema'].keys():
                # Type but not array
                return self.get_example_from_prop_spec(spec['schema'])
            elif 'properties' in spec['schema'].keys():
                # Probably 'object' but not written
    ...
    def get_send_request_correct_body(self, path, action):
        ...
        if path_spec is not None and action in path_spec.keys():
            for name, spec in path_spec[action]['parameters'].items():
                if spec['in'] == 'body':
                    return self.build_example(spec)
